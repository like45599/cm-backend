...
public class UserController {
    ...
    public BaseResponse<Long> userRegister(@RequestBody UserRegisterRequest userRegisterRequest) {
        ...
        String studentNum = userRegisterRequest.getStudentNum();
        if (StringUtils.isAnyBlank(userAccount, userPassword, checkPassword, studentNum)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        long result = userService.userRegister(userAccount, userPassword, checkPassword, studentNum);
    ...
    public BaseResponse<Boolean> updateMyUser(@RequestBody UserUpdateMyRequest userUpdateMyRequest,
        ...
        user.setId(loginUser.getId()); // 确保设置当前登录用户的ID
    ...
    public BaseResponse<ClassOfficers> getClassOfficerByUserId(@PathVariable Long userId) {
        if (userId <= 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }        ClassOfficers classOfficer = classOfficersService.getByUserId(userId);
